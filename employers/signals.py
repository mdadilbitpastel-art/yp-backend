"""Storage cleanup signals for the employers app.

Mirrors `schools.signals` / `about_us.signals` — when a model's FileField/
...
for every employers model that has at least one FileField/ImageField.
"""
...
def register():
    ...
    for model in apps.get_app_config("employers").get_models():
