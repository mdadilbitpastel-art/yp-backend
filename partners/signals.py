"""Storage cleanup signals for the partners app.

Mirrors `employers.signals` / `schools.signals` — when a model's FileField/
...
for every partners model that has at least one FileField/ImageField.
"""
...
def register():
    ...
    for model in apps.get_app_config("partners").get_models():
