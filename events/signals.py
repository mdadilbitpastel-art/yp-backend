"""Storage cleanup signals for the events app.

Mirrors `partners.signals` / `schools.signals` — when a model's FileField/
...
for every events model that has at least one FileField/ImageField.
"""
...
def register():
    ...
    for model in apps.get_app_config("events").get_models():
