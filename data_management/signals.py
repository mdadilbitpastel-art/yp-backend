"""Storage cleanup signals for the data_management app.

Mirrors `schools.signals` / `employers.signals` — when a model's FileField/
...
for every data_management model that has at least one FileField/ImageField.
"""
...
def register():
    ...
    for model in apps.get_app_config("data_management").get_models():
