"""Storage cleanup signals for the about_us app.

Mirrors `home.signals` — when a model's FileField/ImageField is cleared or
replaced, the obsolete file is removed from storage so Cloudinary doesn't
accumulate orphans. Wired up automatically for every about_us model that
has at least one FileField/ImageField.
"""
...
def register():
    ...
    for model in apps.get_app_config("about_us").get_models():
